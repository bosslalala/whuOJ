...
int n,m,L[MAx],R[MAx],key[MAx],npl[MAx],fa[MAx];

//L为左子树编号，R为右子树编号，key为能力值，npl请见上面，fa为并查集数组 
inline int getfunc(int o) { 
    return o==fa[o] ? o : fa[o]=getfunc(fa[o]);
    //路径压缩更快 
}

inline int jin(int a,int b) {
    //合并函数 
    if (!a) return b;//判断是否为空 
    if (!b) return a;
    if (key[a]<key[b]) swap(a,b);//维护堆性质 
    R[a]=jin(R[a],b);//将b合并到a的右子树 
